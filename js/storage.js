class StorageManager {
    ...
    addSong(song, audioData = null) {
        ...
        song.tags = song.tags.split(',').map(tag => tag.trim()).filter(tag => tag);
    ...
    updateSong(id, updatedSong) {
        ...
        if (index !== -1) {
            // Convert tags string to array if provided as string
            if (typeof updatedSong.tags === 'string') {
                updatedSong.tags = updatedSong.tags.split(',').map(tag => tag.trim()).filter(tag => tag);
            }
